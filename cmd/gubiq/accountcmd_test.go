...
func TestAccountListEmpty(t *testing.T) {
	gContractNet := runGContractNet(t, "account")
	gContractNet.expectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gContractNet := runGContractNet(t, "--datadir", datadir, "account")
	defer gContractNet.expectExit()
...
func TestAccountNew(t *testing.T) {
	gContractNet := runGContractNet(t, "--lightkdf", "account", "new")
	defer gContractNet.expectExit()
	gContractNet.expect(`
...
`)
	gContractNet.expectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gContractNet := runGContractNet(t, "--lightkdf", "wallet", "import", "testdata/guswallet.json")
	defer gContractNet.expectExit()
	gContractNet.expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gContractNet.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gContractNet.stderrText(), m) {
...
// https://github.com/ContractNet/go-ContractNet/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gContractNet.setTemplateFunc("keypath", func(file string) string {
