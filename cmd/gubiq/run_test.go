...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gContractNet-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we're the child process for runGContractNet.
...
// spawns gContractNet with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGContractNet(t *testing.T, args ...string) *testgContractNet {
	tt := &testgContractNet{T: t, Executable: os.Args[0]}
	...
	// Boot "gContractNet". This actually runs the test binary but the init function
...
//
//     gContractNet.expect(`Passphrase: {{.InputLine "password"}}`)
func (tt *testgContractNet) InputLine(s string) string {
	io.WriteString(tt.stdin, s+"\n")
	return ""
}

func (tt *testgContractNet) setTemplateFunc(name string, fn interface{}) {
	if tt.Func == nil {
...
// before matching.
func (tt *testgContractNet) expect(tplsource string) {
...
func (tt *testgContractNet) matchExactOutput(want []byte) error {
...
// after expectRegexp.
func (tt *testgContractNet) expectRegexp(resource string) (*regexp.Regexp, []string) {
	var (
...
func (tt *testgContractNet) expectExit() {
	var output []byte
...
func (tt *testgContractNet) interrupt() {
...
// returned.
func (tt *testgContractNet) stderrText() string {
	tt.stderr.mu.Lock()
	defer tt.stderr.mu.Unlock()
	return tt.stderr.buf.String()
}

func (tt *testgContractNet) withKillTimeout(fn func()) {
