...
func AccumulateRewards(statedb *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if header.Number.Cmp(big.NewInt(2866909)) > 0 {
		reward = big.NewInt(7e+18)
	}
	if header.Number.Cmp(big.NewInt(4300363)) > 0 {
		reward = big.NewInt(6e+18)
	}
	if header.Number.Cmp(big.NewInt(5733818)) > 0 {
		reward = big.NewInt(5e+18)
	}
	if header.Number.Cmp(big.NewInt(7167272)) > 0 {
		reward = big.NewInt(4e+18)
	}
	if header.Number.Cmp(big.NewInt(8600727)) > 0 {
		reward = big.NewInt(3e+18)
	}
	if header.Number.Cmp(big.NewInt(15768000)) > 0 {
